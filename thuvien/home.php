...
<body>
    ...
    <div class="d-flex" id="wrapper">
        ...
        <div id="page-content-wrapper">
            ...
            //đếm sách
            $count_sach = "SELECT count(masach) as tong_sach FROM sach";
            $query = mysqli_query($connect, $count_sach);
            $dem_sach = mysqli_fetch_array($query);
            //đếm độcgia
            $count_dg = "SELECT count(madg) as tong_dg FROM docgia";
            $query = mysqli_query($connect, $count_dg);
            $dem_dg = mysqli_fetch_array($query);
            //đếm độcgia
            $count_muon = "SELECT count(mamt) as tong_muon FROM muontra";
            $query = mysqli_query($connect, $count_muon);
            $dem_muon = mysqli_fetch_array($query);
                ...
                <div class="d-flex justify-content-evenly ">
                    <div class="d-flex bg-success bg-gradient rounded">
                        <div class="mt-1 ms-1  row">
                            <h1>
                                Có
                                <?php echo $dem_sach['tong_sach']; ?>
                            </h1>

                            <a class="text-decoration-none" href="book.php">
                                <h2>Sách</h2>
                            </a>
                        </div>
                        <div class="mt-2" style="width: 100px; height: 150px;border-radius:5px;"><i
                                class="bi bi-journal-text" style="font-size:100px; color:gray;"></i>
                        </div>
                    </div>
                    <div class="d-flex bg-danger bg-gradient rounded">
                        <div class="mt-1 ms-1  row">
                            <h1> Có
                                <?php echo $dem_dg['tong_dg']; ?>
                            </h1>
                            <a class="text-decoration-none" href="docgia.php">
                                <h2>Độc Giả</h2>
                            </a>
                        </div>
                        <div class="mt-2" style="width: 100px; height: 150px;border-radius:5px;"><i
                                class="bi bi-person-fill" style="font-size:100px; color:gray;"></i>
                        </div>
                    </div>
                    <div class="d-flex bg-warning bg-gradient rounded">
                        <div class="mt-1 ms-1  row">
                            <h1> Có
                                <?php echo $dem_muon['tong_muon']; ?>
                            </h1>

                            <a class="text-decoration-none" href="ShowSach.php">
                                <h4>Sách Đã Mượn</h4>
                            </a>
                        </div>
                        <div class="mt-2" style="width: 100px; height: 150px;border-radius:5px;"><i
                                class="bi bi-journal-check" style="font-size:100px; color:gray;"></i>
                        </div>
                    </div>
